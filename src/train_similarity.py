		...
		for k1 in range(499):
			...
			for k2 in range(k1+1,500):
				if category in score_ignore:
					...
					feature_k2 = features[num_skip + k2]
				x_temp.append(np.concatenate((feature_k1[np.newasix,:], feature_k2[np.newasix,:]), axis = axis))
				y_temp.append(scores[category][k1,k2])
		x_temp = np.array(x_temp)
		y_temp = np.array(y_temp)
		y_temp_sorted = sorted([(y_temp[k],k) for k in range(len(y_temp))], key = lambda t:t[0])
		idx = np.array([k for _, k in y_temp_sorted[:1000]] + [k for _, k in y_temp_sorted[-1000:]])
		x.append(x_temp[idx])
		y.append(y_temp[idx])
		if category in score_ignore:
			skip += 1
